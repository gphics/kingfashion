...
function EachCartItem({ removeCartItem, item }: { item: styleObjectType, removeCartItem: (id: string) => void; }) {
  ...
  return (
    <div className="each-cart-item">
      <Image  width={200} height={200} alt={name || "item image"} src={images && images[0].secure_url || DefaultImage} />
      <article>
        <h3> {name && name.length < 26 ? `${name}` : `${name && name.slice(0, 26)} ...`} </h3>
        <h3>
          {" "}
          <FaNairaSign /> {price}{" "}
        </h3>
        <Link  className="cart-btn" href={`${whatsappLink}`}>Buy Now</Link>
